...
class BlowFish:
    def __init__(self, key, bitlength=448):
        ...
        if hexkey[-1] == "L":
            hexkey = hexkey[:-1]
        # pad in case of leftmost zeroes
        ...
        for i in range(18):
            ...
            self.parray[i] ^= subkey  # immediate XOR -- Python 2.0+ syntax

        # encrypt 0-data, then keep re-encrypting and reassigning P-boxes
        output = 0
        for i in range(0, 17, 2):
            output = self.bfencrypt(output)
            self.parray[i], self.parray[i + 1] = output >> 32, output & 0xFFFFFFFF
    ...
    def bfencrypt(self, x):
        ...
        for i in range(16):
            xL = xL ^ self.parray[i]  # XOR with P-box
            xR = self.F(xL) ^ xR  # apply F to xL, XOR with xR
        ...
        xR = xR ^ self.parray[16]
        xL = xL ^ self.parray[17]
        return (xL << 32) | xR
    ...
    def F(self, xL):
        ...
        for i in range(4):
            offset = (3 - i) * 8
            qs[i] = (xL & 0xFF << offset) >> offset  # byte i of xL
        # apply function
        rtnval = (self.sbox[0][qs[0]] + self.sbox[1][qs[1]]) % (1 << 32)  # add S1+S2
        rtnval ^= self.sbox[2][qs[2]]  # XOR w/ S3
        return (rtnval + self.sbox[3][qs[3]]) % (1 << 32)  # add S4
    ...
    def encrypt(self, instring):
        ...
        for i in range(blocks + 1):
            inbytes = self.mkchunk(
                instring[i * 8 : i * 8 + 8]
            )  # 8-byte string as hex no.
            cipher = self.bfencrypt(inbytes)  # ...encrypted
            outbytes = self.mkbytes(cipher)
