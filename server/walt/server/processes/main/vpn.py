...
class VPNManager:
    ...
    def update_vpn_dnat(self):
        ...
        for ep_host, ep_port in self.http_ep_history:
            do(
                "iptables -t nat --append WALT-DNAT "
    ...
    def cleanup(self):
        ...
        do(
            "iptables -t nat --delete PREROUTING "
            f"--source {WALT_SUBNET} "
            f"! --destination {WALT_SUBNET} "
            "--jump WALT-DNAT"
        )
    ...
    def enrollment(self, ip, pubkey):
        ...
        if vpn_node is None:
            error_msg = (
                "VPN enrollment request not coming from "
        ...
        if (
            self.get_vpn_entrypoint("ssh") is None
            or self.get_vpn_entrypoint("http") is None
        ):
        ...
        with tempfile.TemporaryDirectory() as tmpdirname:
            ...
            do(
                f"ssh-keygen -s {VPN_CA_KEY} -I '{cert_id}' "
    ...
    def revoke_vpn_auth_key(self, cert_id):
        ...
        run(
            shlex.split(f"ssh-keygen -k -u -s {VPN_CA_KEY} -f {VPN_SERVER_KRL} -"),
            input=f"id: {cert_id}",
            text=True,
        )
        # reload sshd
