...
def handle_registration_request(
    ...
    if image_fullname not in images:
        ...
        logs.platform_log(
            "devices", line=f"Device {db_info.name} is a walt node of type '{model}'."
        )
        logs.platform_log(
            "devices",
            line=(
    ...
    wf_steps += [
        wf_update_device_in_db,
        wf_update_status_manager,
        exports.wf_update_persist_exports,
        images.wf_update_image_mounts,
        wf_dhcpd_named_update,
...
def wf_after_pull_image(wf, pull_result, image_fullname, mac, model, logs, **env):
    if pull_result[0]:
        # ok
        logs.platform_log(
            "devices", line=f"Image {image_fullname} was downloaded successfully."
        )
        wf.next()
    else:
