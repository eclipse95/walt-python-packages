import numpy as np
from numba import i2, njit, u2, u8
...
def _fast_decode_timestamp(last_timestamp, bytecode, pos):
    offset = np.left_shift(
        (bytecode[pos : pos + 4] - 1).astype(np.uint64), TS_LEFT_SHIFTS
    ).sum()
    return offset + last_timestamp


@njit((u8, u8, u8[:], u2, u2), cache=True)
def _fast_apply_linear_timestamps(
    last_ts, new_ts, locations_timestamps, locations_ts_start, locations_ts_end
):
...
def fast_analyse_block(
    ...
    return (
        locations_file_id[:locations_pos],
...
def fast_compute_durations(locations_stack_depths, locations_timestamps, block_ts_end):
    ...
    for location_pos in np.arange(len(locations_stack_depths)):
        while num_pending_indices > 0:
            ...
            if locations_stack_depths[location_pos] <= locations_stack_depths[idx]:
                ...
                durations[idx] = (
                    locations_timestamps[location_pos] - locations_timestamps[idx]
                )
    ...
    for idx in pending_indices[:num_pending_indices]:
        durations[idx] = block_ts_end - locations_timestamps[idx]
    return durations
