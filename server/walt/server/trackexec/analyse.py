...
def trackexec_analyse(trackexec_log_dir, num_items, file_id, sort_field):
    ...
    it = ((tuple(k) + tuple(v[1:]) + (0.0,)) for k, v in reader.src_index.items())
        ...
        for row in a[arr_sort_idx]:
            ...
            rows.append(
                (
                    ...
                    location,
                    source_line,
                    f"{row.cum_duration:.3f}s",
                    f"{row.avg_duration:.3f}s",
...
def sort_column(s):
    if s not in SORT_FIELDS:
        raise ValueError(
            "Sort column must be one of: " + ", ".join(SORT_FIELDS.keys()) + "."
        )
    return s
