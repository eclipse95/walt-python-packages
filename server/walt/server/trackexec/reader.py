...
from walt.server.trackexec.const import MAP_BLOCK_ID_MASK, MAP_BLOCK_ID_SHIFT, OpCodes
from walt.server.trackexec.fast import fast_analyse_block, fast_compute_durations
...
class LogsReader(LogAbstractManagement):
    ...
    def __init__(self, dir_path):
        ...
        self._block_id = 0
        self._index_blocks = np.memmap(
            self._log_index_path, dtype=index_block_dt(), mode="r"
        )
        ...
        if self._src_index is None:
            ...
            if num_added_blocks > 0:
                ...
                while True:
                    ...
                    print(
                        f"Updating source index... {op_num}/{num_added_blocks}\r",
                        end="",
                    )
        ...
        if self._filenames is None:
            with tarfile.open(self._log_sources_archive_path, "r") as t_r:
                self._filenames = list(
                    ("/" + tar_member.name) for tar_member in t_r.getmembers()
                )
        return self._filenames

    def _short_file_location(self, path_items, lineno, maxlen, allow_double_star=True):
        ...
        if len(location) > maxlen:
            ...
            return self._short_file_location(path_items[1:], lineno, maxlen, False)
        return location
    ...
    def seek(self, block_id):
        ...
        if self._block_id >> MAP_BLOCK_ID_SHIFT != block_id >> MAP_BLOCK_ID_SHIFT:
    ...
    def _read_block(self):
        ...
        block = self._block
        source_file_ids, source_linenos, source_stack_depths, source_ts = (
            fast_analyse_block(
                block["stack"]["file_id"],
                block["bytecode"],
                OpCodes.CALL,
    ...
    def _compute_durations(self):
        ...
        durations_data.duration = fast_compute_durations(
            durations_data.stack_depth, durations_data.timestamp, block_ts_end
        )
