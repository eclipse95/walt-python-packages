...
def sw_port_set_poe(sw_ip, sw_port, poe_status, sw_snmp_version, sw_snmp_community):
    snmp_conf = {"version": sw_snmp_version, "community": sw_snmp_community}
    try:
        ...
        if poe_status is False:
            if proxy.poe.check_poe_enabled(sw_port) and not proxy.poe.check_poe_in_use(
                sw_port
            ):
...
def walt_set_poe():
    ...
    try:
        ...
        return
    res = sw_port_set_poe(
        sw_ip, sw_port, poe_status, sw_snmp_version, sw_snmp_community
    )
    if res[0]:
        return
