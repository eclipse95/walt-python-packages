...
def list_valid_auth_keys(context):
    ...
    if len(valid_keys) == 0:
        ...
        if mask_forgotten.sum() > 0:
            ...
            print(
                (
                    auth_keys_forgotten_dev.certid
                    + ": was used by "
...
def revoke_key(context):
    ...
    while True:
        try:
            ...
            cert_id = prompt(
                ("Enter the key you want to revoke " "(or ctrl-C to abort):"), type=str
            )
...
def compute_menu_options(context):
    ...
    options = {
        "generate a VPN SSH entrypoint setup script": generate_ssh_ep_script,
        f"list valid authentication keys (count={num_valid})": list_valid_auth_keys,
        f"list revoked authentication keys (count={num_revoked})": list_revoked_auth_keys,
    }
    if num_valid > 0:
        options.update(
            {
                "revoke an authentication key": revoke_key,
            }
        )
    options.update(
        {
            "quit this screen": quit_screen,
        }
    )
    return options
